...
export function MemberSubjectedTransactions({memberId}: { memberId: string }) {
    ...
    const memberTransactions = useQuery({
        queryFn: async () => {
            const response = R(await client.GET("/api/member-transactions/subject/{memberId}", {
                params: {
                    path: {
                        memberId: memberId
                    },
        ...
        retry: false,
        enabled: doQuery,
        queryKey: [MEMBER_TRANSACTIONS_QUERY_KEY, memberId, "subjected", sortBy, sortDirection, offset]
    });

    return <div className={"flex-1 gap-2 flex flex-col"}>
        <div className={"text-xl font-semibold"}>Transactions done:</div>
        <Table>
            <TableHeader>
                <TableRow>
