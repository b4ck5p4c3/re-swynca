...
export class BaseTransactionSignerService {
  ...
  static getChecksum (data: Buffer): number {
    ...
    for (const byte of data) {
      ...
      for (let index = 0; index < 8; index++) {
        crc = ((crc >>> 1) ^ (0xED_B8_83_20 &
                    ((crc & 1) ? 0xFF_FF_FF_FF : 0x00_00_00_00))) >>> 0
      }
    }
  ...
  async signTransaction (transaction: Buffer): Promise<Buffer> {
    ...
    try {
      ...
      await port.write(Buffer.from(Array.from({ length: 16 }).map(_ => 0)))
      ...
      const [errorCode] = await port.read(1)
      if (errorCode != 0) {
